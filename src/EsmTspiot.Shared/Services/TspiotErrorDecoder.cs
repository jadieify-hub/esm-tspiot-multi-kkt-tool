...
namespace EsmTspiot.Shared.Services
{
    public static class TspiotErrorDecoder
    {
        public static string Decode(int statusCode, string responseBody)
        {
            ...
            {
                return "Ошибка 1012: ЕСМ не смог создать службу второй ККТ автоматически. Создайте службу второй ККТ вручную от имени администратора, затем повторите проверку текущих ККТ.";
            }

            if (ContainsErrorCode(body, 1013))
            {
                return "Ошибка 1013: служба второй ККТ создана, но не запущена. Запустите аварийный фикс службы от имени администратора, чтобы проверить параметры, пересоздать службу при необходимости и запустить её.";
            }
            ...
            {
                return "Служба ЕСМ не зарегистрирована или не запущена. Также проверьте связь драйвера со второй ККТ.";
            }

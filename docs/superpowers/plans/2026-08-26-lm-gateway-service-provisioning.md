...
> **Исторический план:** актуальный as-built полный цикл описан в `docs/superpowers/specs/2026-08-29-managed-local-module-instances-design.md` и одноимённом плане 2026-08-29. Текущий операторский диалог не запрашивает и не хранит логин/пароль: он показывает `softPort` и предзаполненный endpoint ЛМ, после чего один helper/UAC создаёт ЛМ и контроллеры. Привязка/бизнес-инициализация выполняется ЕСМ либо сторонней утилитой. Ниже сохранён первоначальный план контроллерного слоя как архитектурная история; его старые названия кнопок и credential-шаги не являются инструкцией для текущего интерфейса.
